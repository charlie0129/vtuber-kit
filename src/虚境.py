...
def 添加深度信息(所有图层):
    with open('../assets/深度.yaml', encoding='utf8') as f:
        深度信息 = yaml.load(f)
    for 图层信息 in 所有图层:
        if 图层信息['名字'] in 深度信息:
...
def opengl绘图循环(所有图层, psd尺寸):
    ...
    while not glfw.window_should_close(window):
        ...
        横旋转量, 竖旋转量 = 特征缓冲()
        for 图层数据 in 所有图层:
            if 图层数据['名字'] in 现实.目 + 现实.口:
                if 图层数据['名字'] != '表情上/目/' + str(现实.获取眼睛大小等级()) and 图层数据['名字'] != '表情上/口/' + str(现实.获取嘴巴大小等级()):
                    continue
            a, b, c, d = 图层数据['位置']
            z = 图层数据['深度']
